...
import { getAppointmentBillingAmounts } from '../utils/billingUtils';
...
const Appointments = ({
  ...
  const hasLocalPatientConflict = (patientId, date, time, excludeAppointmentId = null) => {
    ...
    return loadedAppointments.some(appt =>
      ...
      appt.date === date &&
      appt.time === time &&
      !['Cancelled', 'Walked Out'].includes(appt.status) &&
  ...
  const handleRebook = (appt) => {
    ...
    setRebookingApptId(appt.date < safeCurrentDate || appt.status === 'Walked Out' ? null : appt._id);
    setIsFollowUpBooking(false);
    setFollowUpSourceApptId('');
    setIsAddModalOpen(true);
  };

  const openFollowUpBooking = (appt) => {
    const doc = getDoctorById(appt.doctorId);
    setNewAppt({
      patientId: getEntityId(appt.patientId),
      department: doc ? doc.department : '',
    ...
    setFollowUpSourceApptId(getEntityId(appt));
    setIsAddModalOpen(true);
  };

  const handleStartReportReview = async (appt) => {
    ...
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) {
        return showNotification(result.error || 'Failed to start report review.', 'error');
      }

      await fetchAllData(true);
      showNotification('Ready for Review', 'success', `${getPatientName(appt.patientId)} is ready for report review.`);
    } catch (err) {
      showNotification('Failed to start report review.', 'error');
    } finally {
  ...
  const handleSaveDraftAndExit = async () => {
    ...
    try {
      ...
      showNotification(
        'Draft Saved',
        'success',
        activeConsultationAppt.activeConsultationMode === 'Report Review'
          ? 'Report review draft can be resumed from this appointment.'
  ...
  const handleDiscardConsultation = async () => {
    ...
    try {
      ...
      showNotification(
        ...
        'success',
        activeConsultationAppt.activeConsultationMode === 'Report Review'
          ? 'Report review draft was discarded.'
  ...
  const renderAppointmentCard = (appt) => {
    ...
    const billingPayments = Array.isArray(appt.billing?.payments) ? appt.billing.payments : [];
    ...
    const isCheckInPaymentFullyPaid = Boolean(
      ...
      (checkInPaymentAmounts.total > 0 && checkInPaymentAmounts.balance <= 0 && checkInPaymentAmounts.paid > 0)
    );
    const paymentRequiredBeforeConsult = data.clinic?.collectConsultationPaymentAtCheckIn === true && uiStatus === 'Checked In' && !appt.reportsReadyAt;
    const requireFullPaymentBeforeConsult = data.clinic?.requireFullPaymentBeforeConsultation === true;
    ...
    const actions = {
      ...
      consult: {
        label: appt.activeConsultationMode === 'Report Review'
          ? 'Resume Report Review'
          : (uiStatus === 'Checked In' && Boolean(appt.reportsReadyAt))
          ? 'Consult'
          : (uiStatus === 'Checked In' && Boolean(appt.reportReviewStartedAt))
          ? 'Consult'
          : (uiStatus === 'Checked In' && hasRecommendedTests && !appt.reportReviewStartedAt)
          ? 'Review Reports'
          : appt.type === 'Follow-Up'
          ? 'Add Follow-Up Note'
          : ((isInConsultation || isDraft || (uiStatus === 'Awaiting Reports' && Boolean(appt.reportsReadyAt))) ? 'Resume Consult' : 'Start Consult'),
        icon: Activity,
        onClick: () => ((appt.activeConsultationMode === 'Report Review' || (uiStatus === 'Checked In' && hasRecommendedTests && !appt.reportsReadyAt && !appt.reportReviewStartedAt)) ? handleStartAddendum(appt) : handleStartConsultation(appt))
      },
