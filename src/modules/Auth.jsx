...
const Auth = ({ authState, setAuthState }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setInvalidFields([]);
    console.log("3. Validation passed. Attempting fetch...");
    try {
    if (authState === 'login') {
    ...
    // Only run this if the user object actually exists
    localStorage.setItem('clinicId', result.user.clinicId._id || result.user.clinicId);
    localStorage.setItem('userName', result.user.name);
    setAuthState('authenticated');
} else {
    // If there's no user object, show the error message from the backend
    setError(result.error || 'Login failed. Please check your credentials.');
}
    } 
    else if (authState === 'forgot') {
      // You can eventually add an API call here for real emails
      ...
      // You can add an API call here to update the password in MongoDB
      setSuccess('Password set successfully!');
      setTimeout(() => setAuthState('login'), 1500);
    }
  } catch (err) {
    console.log("5. Fetch error catch triggered:", err);
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-slate-100 animate-scaleIn">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  type={showPassword ? "text" : "password"} 
                  placeholder="••••••••" 
                  className={`w-full pl-9 pr-10 py-2.5 bg-slate-50 border rounded-xl text-[14px] transition-all outline-none ${invalidFields.includes('confirmPassword') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-200 focus:ring-2 focus:ring-teal-500'}`}
        ...
        {(authState === 'forgot' || authState === 'reset') && (
          <button 
            ...
            className="w-full flex justify-center items-center gap-2 mt-6 text-[13px] font-bold text-slate-500 hover:text-slate-800 transition-colors"
          >
