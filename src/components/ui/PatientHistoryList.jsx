                    ...
                    <div key={visit._id || idx} className={embeddedMarker ? 'group' : 'relative flex flex-col md:flex-row items-start justify-between md:justify-normal md:odd:flex-row-reverse group'}>
                        ...
                        `}>
                            ...
                            >
                                ...
                                {/* ROW 3: Action Button */}
                                    {canExpand && (
                                        <div className={`w-full h-7 text-[12px] font-bold rounded-lg flex items-center justify-center gap-1 transition-colors
                            ...
                            {isExpanded && !isFetching && (
                                <div className="p-3 border-t border-slate-100 bg-slate-50/50 space-y-4 animate-slideDown cursor-default">
                                    ...
                                    {(visit.vitals?.bp || visit.vitals?.temp || visit.vitals?.weight) && (
                                        <div>
                                            ...
                                            <div className="flex gap-4 bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                                                {visit.vitals.bp && <div><span className="text-[12px] text-slate-400 uppercase font-bold block">BP</span><span className="text-[12px] font-bold text-slate-800">{visit.vitals.bp}</span></div>}
                                                {visit.vitals.temp && <div><span className="text-[12px] text-slate-400 uppercase font-bold block">Temp</span><span className="text-[12px] font-bold text-slate-800">{visit.vitals.temp}°F</span></div>}
                                                {visit.vitals.weight && <div><span className="text-[12px] text-slate-400 uppercase font-bold block">Weight</span><span className="text-[12px] font-bold text-slate-800">{visit.vitals.weight} kg</span></div>}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {(visit.complaints || visit.diagnosis) && (
                                        <div>
                                            ...
                                            <div className="bg-white p-2 rounded-lg border border-slate-100 shadow-sm space-y-2">
                                                {visit.complaints && <div><span className="text-[12px] text-slate-400 uppercase font-bold block">Complaints</span><span className="text-[12px] font-medium text-slate-800">{visit.complaints}</span></div>}
                                                {visit.diagnosis && <div><span className="text-[12px] text-slate-400 uppercase font-bold block">Diagnosis</span><span className="text-[12px] font-bold text-teal-800">{visit.diagnosis}</span></div>}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {visit.medicines && visit.medicines.length > 0 && (
                                        <div>
                                            <h4 className="text-[12px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 flex justify-between items-center"><span className="flex items-center gap-1.5"><Pill size={12} /> Rx</span><span className="text-[12px] text-teal-600 bg-teal-50 px-1.5 rounded font-bold">{visit.medicines.length}</span></h4>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-1 bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                                                {visit.labTests.map((test, idx) => (
                                                    <span key={idx} className="bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded text-[12px] font-medium border border-blue-100">{test.name}</span>
                                                ))}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {onRefillRx && visit.medicines && visit.medicines.length > 0 && (
                                        <button type="button" onClick={() => onRefillRx(visit.medicines)} className="w-full py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold text-[12px] rounded-lg transition-colors flex justify-center items-center gap-1.5 mt-2 border border-blue-200">
